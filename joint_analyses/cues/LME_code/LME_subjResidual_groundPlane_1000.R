...
df <- read.csv('1000_groundPlane_participantData_for_lme.csv')
df$subjID <- factor(df$subjID) 
df$duration <- factor(df$duration)
df$stimulus <- factor(df$stimulus) 

df_grouped <- read.csv('1000_groundPlane_participantData_grouped_for_lme.csv')
...
groundPlane_plot <- ggplot() + 
  ...
  #3
  geom_point(data = gp_effects_abs_df, aes(x = groundPlane, y = fit), color = "darkviolet") +
  #4
  geom_line(data = gp_effects_abs_df, aes(x = groundPlane, y = fit), color = "darkviolet") +
  #5
  geom_ribbon(data = gp_effects_abs_df, aes(x = groundPlane, ymin = lower, ymax = upper), alpha = 0.3, fill = "darkviolet") +
  #6
